...
Timeline events carry both lifecycle intent (`target_status`) and the active assignment target at the time of the update. The `jobs` row still keeps the current assignee and cached status for filtering, but the event stream is the more truthful record of what happened and who the work was pointed at when it happened. The same `reason_code` spine now also captures explicit after-hours handoff receipt plus post-visit accountability signals such as liability review, charge notice, resident appeal, and charge resolution without pretending the repo already has separate Phase 1 charge or visit entities.
